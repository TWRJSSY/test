...
import statusbar.lyric.thirteen.R
import statusbar.lyric.thirteen.config.ActivityOwnSP.config
import statusbar.lyric.thirteen.tools.ActivityTools
import statusbar.lyric.thirteen.tools.ActivityTools.changeConfig
import statusbar.lyric.thirteen.tools.Tools.isNotNull
...
fun ExtendPage(
    ...
    // 重构版新增开关
    val extraKillMusicHeadsUp = remember { mutableStateOf(config.extraKillMusicHeadsUp) }
    val extraKillLockscreenMedia = remember { mutableStateOf(config.extraKillLockscreenMedia) }
    val extraIconRule = remember { mutableStateOf(config.extraIconRule) }
    val extraLockscreenLyric = remember { mutableStateOf(config.extraLockscreenLyric) }
    val lyricRedundancyHide = remember { mutableStateOf(config.lyricRedundancyHide) }
    val videoBlacklistEnabled = remember { mutableStateOf(config.videoBlacklistEnabled) }
    // 重构版新增：默认关闭开关
    val showCustomClock = remember { mutableStateOf(config.showCustomClock) }
    val showLyricIcon = remember { mutableStateOf(config.showLyricIcon) }
    val extraKillMediaCard = remember { mutableStateOf(config.extraKillMediaCard) }
        ...
        LazyColumn(
            ...
            item {
                Column(Modifier.padding(top = 6.dp)) {
                    ...
                    SmallTitle(text = stringResource(R.string.extra_features))
                    Card(
                        modifier = Modifier
                            .fillMaxWidth()
                            .padding(horizontal = 12.dp)
                        ...
                        SuperSwitch(
                            title = stringResource(R.string.show_custom_clock),
                            summary = stringResource(R.string.show_custom_clock_summary),
                            checked = showCustomClock.value,
                            onCheckedChange = {
                                showCustomClock.value = it
                                config.showCustomClock = it
                                    changeConfig()
                            }
                        )
                        SuperSwitch(
                            title = stringResource(R.string.show_lyric_icon),
                            summary = stringResource(R.string.show_lyric_icon_summary),
                            checked = showLyricIcon.value,
                            onCheckedChange = {
                                showLyricIcon.value = it
                                config.showLyricIcon = it
                                    changeConfig()
                            }
                        )
                        SuperSwitch(
                            title = stringResource(R.string.extra_kill_media_card),
                            summary = stringResource(R.string.extra_kill_media_card_summary),
                            checked = extraKillMediaCard.value,
                            onCheckedChange = {
                                extraKillMediaCard.value = it
                                config.extraKillMediaCard = it
                                    changeConfig()
                            }
                        )
                        SuperSwitch(
                            title = stringResource(R.string.extra_kill_music_headsup),
                            checked = extraKillMusicHeadsUp.value,
                            onCheckedChange = {
                                extraKillMusicHeadsUp.value = it
                                config.extraKillMusicHeadsUp = it
                                    changeConfig()
                            }
                        )
                        SuperSwitch(
                            title = stringResource(R.string.extra_kill_lockscreen_media),
                            checked = extraKillLockscreenMedia.value,
                            onCheckedChange = {
                                extraKillLockscreenMedia.value = it
                                config.extraKillLockscreenMedia = it
                                    changeConfig()
                            }
                        )
                        SuperSwitch(
                            title = stringResource(R.string.extra_icon_rule),
                            checked = extraIconRule.value,
                            onCheckedChange = {
                                extraIconRule.value = it
                                config.extraIconRule = it
                                    changeConfig()
                            }
                        )
                        SuperSwitch(
                            title = stringResource(R.string.extra_lockscreen_lyric),
                            checked = extraLockscreenLyric.value,
                            onCheckedChange = {
                                extraLockscreenLyric.value = it
                                config.extraLockscreenLyric = it
                                    changeConfig()
                            }
                        )
                        SuperSwitch(
                            title = stringResource(R.string.lyric_redundancy_hide),
                            checked = lyricRedundancyHide.value,
                            onCheckedChange = {
                                lyricRedundancyHide.value = it
                                config.lyricRedundancyHide = it
                                    changeConfig()
                            }
                        )
                        SuperSwitch(
                            title = stringResource(R.string.video_blacklist_enabled),
                            checked = videoBlacklistEnabled.value,
                            onCheckedChange = {
                                videoBlacklistEnabled.value = it
                                config.videoBlacklistEnabled = it
                                    changeConfig()
                            }
                        )
                    }
