...
package statusbar.lyric.thirteen.config

import android.content.SharedPreferences
import de.robv.android.xposed.XSharedPreferences
import statusbar.lyric.thirteen.BuildConfig
import statusbar.lyric.thirteen.tools.ConfigTools
...
class Config {
    ...
    // ── 重构版新增属性 ──
    // ── 重构版新增开关（默认值与 StateController.loadSwitches 保持一致）──

    /** 挖孔右侧自定义时钟，默认关闭，用户手动开启（手册规定） */
    var showCustomClock: Boolean
        get() = config.opt("show_custom_clock", false)
        set(value) { config.put("show_custom_clock", value) }

    var showLyricIcon: Boolean
        get() = config.opt("show_lyric_icon", true)
        set(value) { config.put("show_lyric_icon", value) }

    var lyricRedundancyHide: Boolean
        get() = config.opt("lyric_redundancy_hide", true)
        set(value) { config.put("lyric_redundancy_hide", value) }

    var videoBlacklistEnabled: Boolean
        get() = config.opt("video_blacklist_enabled", true)
        set(value) { config.put("video_blacklist_enabled", value) }

    /** 屏蔽 QS 媒体大卡片（extra_kill_media_card），默认开启 */
    var extraKillMediaCard: Boolean
        get() = config.opt("extra_kill_media_card", true)
        set(value) { config.put("extra_kill_media_card", value) }

    var extraKillMusicHeadsUp: Boolean
        get() = config.opt("extra_kill_music_headsup", false)
        set(value) { config.put("extra_kill_music_headsup", value) }

    var extraKillLockscreenMedia: Boolean
        get() = config.opt("extra_kill_lockscreen_media", false)
        set(value) { config.put("extra_kill_lockscreen_media", value) }

    var extraIconRule: Boolean
        get() = config.opt("extra_icon_rule", false)
        set(value) { config.put("extra_icon_rule", value) }

    var extraLockscreenLyric: Boolean
        get() = config.opt("extra_lockscreen_lyric", false)
        set(value) { config.put("extra_lockscreen_lyric", value) }
